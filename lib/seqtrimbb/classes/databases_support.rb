  ...
  # Check point (Is not previously updated OR Current fastas != Old fastas)
   ...
   #UPDATE INDEX
	def update_index(databases,info,bbtools)		
		...
		# RAM for database index update, in Mb
		if ENV['RAMDB']
			ramdb = ENV['RAMDB']
		else # otherwise use SEQTRIM_PATH + DB
			ramdb = (info[db_name]['size']/2.0**20).round(0)*30 + 820
		end
			cmd = bbtools.load_bbsplit({'ram' => ramdb, 'cores' => 1,'ref' =>  info[db_name]['path'], 'path' => info[db_name]['index'], 'in' => nil, 'out' => nil, 'int' => nil, 'redirection' => ['2>',info[db_name]['update_error_file']]})
