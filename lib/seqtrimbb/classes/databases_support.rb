   ...
   #UPDATE INDEX
	def update_index(databases,info,bbtools)		
		...
		# Loading BBtools module and cmd execution
			cmd = bbtools.load_bbsplit({'ram' => (info[db_name]['size']/2.0**20).round(0)*30 + 820, 'cores' => 1,'ref' =>  info[db_name]['path'], 'path' => info[db_name]['index'], 'in' => nil, 'out' => nil, 'int' => nil, 'redirection' => ['2>',info[db_name]['update_error_file']]})
