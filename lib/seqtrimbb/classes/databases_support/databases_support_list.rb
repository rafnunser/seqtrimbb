...
class DatabasesSupportList < DatabasesSupport
	...
	def initialize(list,info)
		...
		if list.empty?
			...
			if !rejected.empty?
				rejected.each { |r| puts "Database #{r} is not present" if r != 'all'}
				sto_list << 'all'
			end
			#Add dbs
			list.each { |d| to_list << d if !rejected.include?(d) }
		end
		...
		to_list.each do |db_name|
			case db_name
				when 'all'
					...
					puts "-"
					all_databases.sort.each { |d| puts d }
					puts ""
					puts "Databases provided by SeqTrimBB:"
					puts "-"
					@@repo_info['databases'].sort.each { |d| puts d }
					puts ""
					puts "Indexed databases:"
					puts "-"
					info['indexed_databases'].sort.each { |d| puts d } if (info.key?('indexed_databases') && !info['indexed_databases'].empty?)
					puts "---"
				else
					puts "Entries at #{db_name} database:"
					puts "-"
					if info.key?(db_name)
						info[db_name]['fastas'].each { |f| puts File.basename(f) }
					else
						Dir[File.join(dir,'fastas',db_name,"*.fasta*")].sort.each { |f| puts File.basename(f) }
					end
					puts ""                        
			end
		end
