...
class PluginPolyAt < Plugin
 ...
 #Get report!
	def get_report(json,data)
		super
		with = json['plugin_poly_at']['sequences_with_poly_at']          
		without = json['sequences']['input_count'] - with
		data['plugin_poly_at_percent'] = [
			['content','count'],
			['without poly A/T',without],
			['with poly A/T',with]
		]         
		@plugin << %(           <div style="overflow: hidden">)
		@plugin << %(             <%=pie(id:'plugin_poly_at_percent', header: true, row_names: true, title:"Poly A/T presence", responsive: false, config: { 'pieSegmentPrecision' => 2 })%>)
		@plugin << %(           </div>)
		return @plugin
	end 
