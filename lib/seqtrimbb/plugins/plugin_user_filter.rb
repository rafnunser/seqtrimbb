...
class PluginUserFilter < Plugin
 ...
 #Get report!
	def get_report(json,data)
		super
		with = json['plugin_user_filter']['filtered_sequences_count']          
		without = json['sequences']['input_count'] - with
		data['plugin_user_filter_percent'] = [
			['content','count'],
			['nonfiltered reads',without],
			['filtered reads',with]
		]          
		data['plugin_user_filter_ids'] = []
		data['plugin_user_filter_ids'] << json['plugin_user_filter']['filtering_ids'].keys
		data['plugin_user_filter_ids'] << json['plugin_user_filter']['filtering_ids'].values
		@plugin << %(           <div style="overflow: hidden" class="flex-row">)
		@plugin << %(            <div class="flex-canvas-50">)
		@plugin << %(               <%=pie(id:'plugin_user_filter_percent', header: true, row_names: true, title:"Filtered reads presence", config: { 'pieSegmentPrecision' => 2 })%>)
		@plugin << %(             </div>)
		@plugin << %(             <div class="flex-canvas-50">)
		@plugin << %(               <%=barplot(id:'plugin_user_filter_ids', header: true, title:"Filtered reads", x_label: 'Nreads', config: { 'showLegend' => false })%>)
		@plugin << %(             </div>)
		@plugin << %(           </div>)
		return @plugin
	end      
