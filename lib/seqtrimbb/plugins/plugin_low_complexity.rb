...
class PluginLowComplexity < Plugin
 ...
 #Get report!
	def get_report(json,data)
		super
		@plugin << %(           <p style="text-align: center;">Number of low complexity reads discarded: <b style="font-size: large;">#{json['plugin_low_complexity']['low_complexity_discarded_reads']}</b></p>)
		return @plugin
	end
