      ...
      def test_plugin_adapters
          ...
          #SETUP
           ...
           # Single-ended sample
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx100m -Xms100m in=stdin.fastq out=stdout.fastq int=f ref=#{adapters_db} k=15 mink=8 hdist=1 ktrim=r stats=#{r_outstats} 2> #{r_outstats2} | java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx100m -Xms100m in=stdin.fastq out=stdout.fastq int=f ref=#{adapters_db} k=21 mink=11 hdist=1 ktrim=l stats=#{l_outstats} 2> #{l_outstats2}"             
           ...
           # Saving singles
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx100m -Xms100m in=stdin.fastq out=stdout.fastq int=f ref=#{adapters_db} k=15 mink=8 hdist=1 ktrim=r stats=#{r_outstats} outs=#{outsingles3} 2> #{r_outstats2} | java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx100m -Xms100m in=stdin.fastq out=stdout.fastq int=f ref=#{adapters_db} k=21 mink=11 hdist=1 ktrim=l stats=#{l_outstats} outs=#{outsingles5} 2> #{l_outstats2}"
             manager = PluginManager.new('PluginAdapters',params,bbtools,stbb_db)
             manager.check_plugins_params   
             manager.execute_plugins
             plugin_cmd = manager.plugin_result['PluginAdapters']['cmd']
