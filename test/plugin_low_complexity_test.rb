...
class PluginLowComplexityTest < Minitest::Test
      ...
      def test_plugin_lowcomplexity
           ...
           # Single-ended sample
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx50m -Xms50m in=stdin.fastq out=stdout.fastq int=f entropy=0.01 entropywindow=50 minlength=50 2> #{outstats}"             
           ...
           # Saving singles
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx50m -Xms50m in=stdin.fastq out=stdout.fastq int=t entropy=0.01 entropywindow=50 minlength=50 outs=#{outsingles} 2> #{outstats}"             
             manager = PluginManager.new('PluginLowComplexity',params,bbtools,stbb_db)
             manager.check_plugins_params   
             manager.execute_plugins
             plugin_cmd = manager.plugin_result['PluginLowComplexity']['cmd']
          ...
          # Minlength < 50
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx50m -Xms50m in=stdin.fastq out=stdout.fastq int=t entropy=0.01 entropywindow=40 minlength=40 outs=#{outsingles} 2> #{outstats}"             
             manager = PluginManager.new('PluginLowComplexity',params,bbtools,stbb_db)
             manager.check_plugins_params   
             manager.execute_plugins
             plugin_cmd = manager.plugin_result['PluginLowComplexity']['cmd']
          ...
          #Aditional params
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx50m -Xms50m in=stdin.fastq out=stdout.fastq int=t entropy=0.01 entropywindow=40 minlength=40 outs=#{outsingles} add_param=test 2> #{outstats}"             
             manager = PluginManager.new('PluginLowComplexity',params,bbtools,stbb_db)
             manager.check_plugins_params   
             manager.execute_plugins
             plugin_cmd = manager.plugin_result['PluginLowComplexity']['cmd']
