...
class PluginVectorsTest < Minitest::Test
      ...
      def test_plugin_vectors
          ...
          # Adding aditional params
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} align2.BBSplitter ow=t fastareadlen=500 minhits=1 maxindel=20 qtrim=rl untrim=t trimq=6 t=1 -Xmx#{ram}m in=stdin.fastq int=t minratio=0.80 path=#{stbb_db.get_info(db,'index')} refstats=#{c_outstats} add_param=test outu=stdout.fastq 2> #{c_outstats2} | java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx51m -Xms51m in=stdin.fastq out=stdout.fastq int=t ref=#{vectors_db} k=31 mink=11 hdist=1 ktrim=r stats=#{r_outstats} tbo tpe add_param=test restrictleft=58 restrictright=58 2> #{r_outstats2} | java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx51m -Xms51m in=stdin.fastq out=stdout.fastq int=t ref=#{vectors_db} k=31 mink=11 hdist=1 ktrim=l stats=#{l_outstats} tbo tpe add_param=test restrictleft=58 restrictright=58 2> #{l_outstats2}"
           ...
           # Triming mode: right
             ...
             result = "java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} align2.BBSplitter ow=t fastareadlen=500 minhits=1 maxindel=20 qtrim=rl untrim=t trimq=6 t=1 -Xmx#{ram}m in=stdin.fastq int=t minratio=0.80 path=#{stbb_db.get_info(db,'index')} refstats=#{c_outstats} add_param=test outu=stdout.fastq 2> #{c_outstats2} | java -Djava.library.path=#{nativelibdir} -ea -cp #{classp} jgi.BBDuk t=1 -Xmx51m -Xms51m in=stdin.fastq out=stdout.fastq int=t ref=#{vectors_db} k=31 mink=11 hdist=1 ktrim=r stats=#{r_outstats} tbo tpe add_param=test restrictleft=58 restrictright=58 2> #{r_outstats2}"             
             manager = PluginManager.new('PluginVectors',params,bbtools,stbb_db)
             manager.check_plugins_params   
             manager.execute_plugins
             plugin_cmd = manager.plugin_result['PluginVectors']['cmd']
